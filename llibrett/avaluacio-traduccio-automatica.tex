...
capítol~\ref{se:TiTA}, la noció central és la de \emph{propòsit} de la
traducció.

Per exemple, l'avaluació d'un sistema que s'usa per a la
\emph{disseminació} de textos s'ha de fer estimant d'alguna manera
l'esforç de postedició ja que el sistema serà més útil com més reduït
siga aquest esforç; en canvi, l'avaluació d'un sistema usat per a
l'\emph{assimilació} d'informació s'ha de fer de manera diferent: ací
la utilitat està relacionada més amb la intel·ligibilitat, i es podria
detrminar directament a través de qüestionaris de comprensió
\citep{jones2007} o similars \citep{oregan13p,ageeva15p} o
indirectament, per exemple, estudiant l'èxit a l'hora d'executar una
tasca amb les instruccions traduïdes automàticament
\citep{doherty12p}.

\item \emph{La facilitat d'ús del sistema de TA mateix}: per exemple,
  ``la facilitat amb què es poden crear i actualitzar diccionaris,
  posteditar els textos, controlar el llenguatge d'entrada'' o
  ``l'extensibilitat [del sistema] a parells nous d'idiomes o a noves
  temàtiques'' \citep{hutchins96u}. 
\end{enumerate}
\end{enumerate}
...
\begin{persabermes}{avaluació predictiva}
...
% L'avaluació predictiva pot tenir, a
%   més, un paper fonamental en l'educació dels futurs traductors
%   \citep{mira98j,forcada00p}, en vista de la disponibilitat creixent
%   de sistemes comercials (per exemple, a través d'Internet).
