...
\fi
%\usepackage{graphicx}
%\newcommand{\comprof}[1]{\begin{quote}{\small\sf #1}\end{quote}}
\newcommand{\comprof}[1]{}
\newcommand{\pair}[2]{(\texttt{#1},\texttt{#2})}%
\pagestyle{headings} % canviar a alguna classe de fancyhdr
\ifthenelse{\equal{\aquestllibre}{IAT}}{
\markboth{Informàtica Aplicada a la Traducció: notes de classe}
         {Informàtica Aplicada a la Traducció: notes de classe}
}{}
\ifthenelse{\equal{\aquestllibre}{TAFA}}{
\markboth{Traducció automàtica: fonaments i aplicacions}
         {Traducció automàtica: fonaments i aplicacions}
}{}

% Coses de Juan Antonio:
\usepackage{wasysym}
%\newenvironment{ja}{\usefont{OT1}{ppl}{m}{n}\fbox{\gluon}}{\astrosun}
\newenvironment{ja}{}{} 
...
\ifthenelse{\equal{\aquestllibre}{IAT}}
{
\title{Informàtica Aplicada a la Traducció: \\ notes de classe 
amb exercicis i problemes resolts}
\author{Mikel L. Forcada i Juan Antonio Pérez Ortiz\\
\{\texttt{mlf},\texttt{japerez}\}\texttt{@dlsi.ua.es} \\
\internet{http://www.dlsi.ua.es/~mlf} \\\internet{http://www.dlsi.ua.es/~japerez} \\
...
\noindent Copyright (c) 2004, 2006, 2007 Mikel L. Forcada \& Juan Antonio Pérez Ortiz \\[0.3cm]

\noindent Permission is granted to copy, distribute and/or modify this
document under the terms of the GNU General Public License, version 2.
A
copy of the license may be found at \url{http://www.gnu.org/licenses/gpl.txt}. \\[0.3cm]
...
Pública de GNU, versió 2. Es pot trobar una còpia d'aquesta llicència en \url{http://ca.dodds.net/gnu/gpl.ca.html}.
}{}

\ifthenelse{\equal{\aquestllibre}{TAFA}}
{
\title{Traducció automàtica: fonaments i aplicacions \\ notes
  de classe }
\author{Mikel L. Forcada \\
{\tt mlf@dlsi.ua.es} \\
{\tt http://www.dlsi.ua.es/\~{}mlf} \\
Departament de Llenguatges i Sistemes Informàtics\\
Universitat d'Alacant\\
E-03071 Alacant
}
...
\com{Lista (incompleta) de coses a fer:
  ...
  \item Integrar problemes resolts d'examens de prova i exercicis dels
    tests. \jacom{Fet! Encara que cal repassar-los...}
...
Aquestes pàgines contenen una bona part dels continguts\footnote{Hi ha
...
l'assignatura \emph{Informàtica Aplicada a la Traducció} que cursarà l'alumnat
de tercer curs de la llicenciatura en Traducció i Interpretació de la
Universitat d'Alacant; també poden ser útils per a assignatures similars en
altres universitats (per això s'hi ha inclòs material que no s'estudia en
Informàtica Aplicada a la Traducció).  La lectura d'aquestes notes ---que
...
text i es llisten en la bibliografia.
  ...
  Veureu que els continguts d'aquestes notes es poden dividir en tres
  ...
  (capítol~\ref{se:basesdades}); la segona és una introducció a alguns
  aspectes generals de la traducció automàtica (capítols~\ref{se:TiTA}
  a \ref{se:ASTA}), i la tercera, un recull d'aspectes més
...
l'alumnat de l'assignatura com a altres persones que volen saber sobre el
...
aquest text. Són massa gent per a esmentar-los tots.


En el servidor \url{http://projectes.lafarga.cat/projects/llibreiat} trobareu els fitxers
font (\LaTeX, .eps, etc.) necessaris per a tornar a generar el llibre, de
manera que, si ho desitgeu, els podeu modificar per a generar un text
nou i publicar-lo vosaltres, però sempre d'acord amb les condicions de
la Llicencia General Pública  de GNU (descrita en
\url{http://www.gnu.org/licenses/gpl.txt}). Aquesta llicència us obliga,
bàsicament, a publicar qualsevol treball derivat d'aquest amb la
mateixa llicència. Així garantim que el nostre treball està sempre
...
\com{Caldrà canviar el text de la introducció perquè hi ha coses que
  ja estaran fetes, afegir agraïments (Gema), etc.}


Aquestes notes contenen una part dels continguts\footnote{Hi ha
  continguts ---diguem-ne millor habilitats--- que s'aprenen com a
  part de les sessions de laboratori i que no figuren en aquest
  document.} de l'assignatura \emph{Traducció Automàtica, Fonaments i
  Aplicacions} que cursa l'alumnat del programa de doctorat
\emph{Aplicacions de la Informàtica} del Departament de Llenguatges i
Sistemes Informàtics de la Universitat d'Alacant.  La lectura
...
De fet, aquestes notes són provisionals. Es probable que al final del
curs de doctorat es publiquen unes notes millorades. Heus ací una
llista d'algunes coses que crec que s'hi haurien d'afegir:
\begin{itemize}
\item En l'apartat~\ref{s3:anmor}, una descripció més rigorosa dels
  analitzadors morfològics basats en transductors d'estats finits; pel
  moment, us recomane que consulteu \citep{roche97b}
\citep{mohri97j} o \citep{garrido99j}.
\item En l'epígraf~\ref{s3:alcat}, una definició més detallada del
  model de desambiguació lèxica basat en $n$-grames.
\item Un apartat nou dedicat a la traducció automàtica estadística.
\item Referències de suport sobre lingüística per als informàtics.
\end{itemize}
A més, potser s'hauria d'usar una notació més formal, és a dir, més
pròpia de l'alumnat d'un programa de doctorat d'informàtica.
 
Veureu que els continguts d'aquestes notes es poden dividir en dues
parts: la primera és una introducció a alguns aspectes generals de la
traducció automàtica (capítols~\ref{se:TiTA} a \ref{se:ASTA}) i la
segona, un recull d'aspectes més concretament referits a la traducció
espanyol--català (capítols~\ref{se:PdTACC} i \ref{se:ETACC}), els
quals poden servir com a il·lustració de l'estudiat en un cas concret.
Els continguts d'aquesta segona part són, per tant, complementaris.  }
{}
...
\com{Llista de coses per fer en aquest capítol:
  ...
  amb sistemes diferents de Windows). \jacom{Crec que no cal arribar
  tan lluny!}
...
el teclat, els xips\footnote{El xip \'{e}s l'element bàsic de la
...
\item[Programari] (o \emph{software}): un o m\'{e}s \emph{programes}
  ...
  exemple, un processador de textos com Microsoft Word o WordPerfect
  pot estar compost per m\'{e}s d'un \emph{programa}. Un
  \emph{programa} \'{e}s una seqüència (conjunt ordenat)
  d'ins\-truc\-ci\-ons que són seguides o executades pel
...
\item[Entrada:] la funció primària dels dispositius d'entrada
  ...
  informació. El dispositiu d'entrada m\'{e}s comú \'{e}s el
  teclat. N'hi ha d'altres, com el ratolí o la maneta de jocs, o
  l'escànner, un dispositiu que llegeix una imatge impresa i la
  converteix en un fitxer (vegeu l'apartat~\ref{ss:preedposted}) que
...
en dos grups:
        ...
        \item[Memòria primària:] memòria ràpida de
          ...
          mentre l'ordinador està funcionant. Normalment
    ...
    \item[Memòria secundària:] memòria de llarg termini,
      permanent.  Exemples: disquets, discos fixos o durs, unitats de
      cinta, memòries USB (també anomenades llàpis o pendrive) i diverses formes de ROM (\emph{read-only memory},
      memòria de lectura nom\'{e}s), com els xips ROM o el CD-ROM.
      Els disquets, els discos fixos i les cintes són dispositius
      d'emmagatzematge magnètic, poc m\'{e}s o menys com ho
      són les cassets. La memòria USB és un dispositiu de memòria flaix, un xip de memòria  que manté el seu contingut en absència d'alimentació, que es connecta al port USB de l'ordinador. La memòria ROM sol estar feta de xips
      ...
      darrera generació de sistemes d'emmagatzematge basat en discs
      òptics; bàsicament, es tracta d'un CD més ràpid i amb més
      capacitat. Només uns anys després de la seua introducció ha
      esdevingut el sistema estàndar per a vídeo i aviat
      ho serà també per a
      dades i música. \end{ja}
    
    La manera més comuna d'emmagatzemar les dades en memòria
    secundària estiguen organitzades en {\em fitxers} o
...
personals, la unitat central \'{e}s un únic xip de silici.
...
d'eixida són les
impressores, les tra\c{c}adores gràfiques, etc.
...
\paragraph{Tipus de programari:} Hi ha tres classes bàsiques de programari:
...
que s'emmagatzema permanentment en xips ROM.
...
d'interaccionar amb l'usuari, els sistemes operatius es poden dividir
en \emph{gràfics}, és a dir, basats en ratolí, punters, finestres,
etc. (Windows, Macintosh OS, OS/2) i de {\em línia d'ordres}, és a
dir, basats en text (Unix primigeni, VAX/VMS, MS-DOS). Els sistemes
poden ser tamb\'{e} \emph{monousuari} (MS-DOS, Windows 3.11) o
\emph{multiusuari} (Unix, VAX/VMS), si poden o no donar acc\'{e}s i
suport a m\'{e}s d'un usuari alhora, o \emph{monotasca} (MS-DOS) i
\emph{multitasca} (quasi tots els altres), si poden o no executar
m\'{e}s d'un programa alhora. Hi havia tamb\'{e} sistemes operatius
especialitzats per a connectar ordinadors formant una xarxa, com
Novell Netware, però són poc usats perquè els sistemes operatius
actuals ja estan preparats per a les xarxes m\'{e}s
bàsiques\footnote{L'organització dels ordinadors en una xarxa permet
  la comunicació d'informació entre ells i la compartició de recursos,
  com ara una impressora. Internet (vegeu el
...
\item[Processadors de llenguatges de programació:] les
  ...
  potents (com ara BASIC, FORTRAN, C, Pascal) i usen programes
  ...
  l'1 al 10 quedaría així en Pascal:
\begin{verbatim}
program SUMA;
var
   index, suma: integer;
...
necessitats dels usuaris. Se'n podrien fer dos grups:
        ...
        classes d'usuaris. Ací en teniu alguns exemples:
                \begin{description}
                \item[Processadors de text] per a preparar, modificar,
              ...
              \item[Navegadors d'Internet:] programes que permeten
                ...
                màquines connectades a aquesta xarxa.\footnote{El
                  nom \emph{navegador} s'usa per l'analogia
                  ---dèbil--- existent entre els mecanismes
                  d'acc\'{e}s als documents de la Internet i la
                  navegació mitjan\c{c}ant un mapa en una zona
                  desconeguda. Altres noms: \emph{browser}
                  (fullejador), \emph{explorador} i \emph{visor}.}
                Exemples: \emph{Microsoft Internet Explorer},
                \emph{Netscape}, \emph{Mozilla}, \emph{Opera}, o
                l'històric \emph{Mosaic} de mitjans dels noranta.
                Vegeu el capítol~\ref{se:Internet}.\label{pg:navegadors}
...
\jacom{He llevat tot allò dels mainframes, miniordinadors, etc. perquè
  està passat de moda. Propose aquesta classificació:}
...
\item[Portàtils] (en anglés \emph{laptop} i de vegades
  \emph{notebook}): Tenen una grandària menor que la d'un maletí i un
  pes lleuger que permet dur-los sense massa esforç d'un lloc a un
  altre. Però, el seu volum reduït limita les possibilitats de fer-hi
  ampliacions i, per tant, el seu temps de vida és sovint més curt que
  el dels ordinadors de sobretaula. Tot i que fa uns anys el seu preu
  era molt superior al d'un ordinador de sobretaula, avui la
  diferència és cada vegada més reduïda.
\item[De mà o de butxaca] (en anglés \texttt{handheld}): Tenen una
  grandària una mica més gran que la d'una calculadora i poden
  considerar-se hereus de les antigues agendes electròniques, tot i
  que els ordinadors de mà (coneguts també com a \emph{PDA} per
  \emph{Personal Digital Assistant}, assistent digital personal)
  porten un sistema operatiu potent que permet fer-los servir en
  tasques como ara l'edició de documents o la navegació per Internet a
  través d'una connexió sense fils.  Com que sovint no porten teclat,
  els caràcters s'hi introdueixen escrivint-los directament sobre la
  seua pantalla tàctil mitjançant un punter. Els ordinadors de mà es
  poden connectar (amb cables o mitjançant tecnologies sense cables) a
  altres dispositius (ordinadors, telèfons mòbils, impressores, etc.)
  per compartir dades.
\item[Servidors:] Els servidors són ordinadors que contenen i
  ...
  tenir pantalles, teclats o ratolins.  Aquests ordinadors es poden
  presentar en grups connectats entre sí per a oferir major potència i
  capacitat i es munten de vegades en armaris especials anomenats
  \emph{racks}.

\end{description}

D'altra banda, sovint es fa la distinció entre els ordinadors de tipus
PC i els Macinstosh (sovint anomenats Mac); els PC són l'evolució
dels primers ordinadors personals desenvolupats per IBM, tot i que
actualment són fabricats per un nombre molt gran d'empreses; els Mac
són fabricats exclusivament per l'empresa Apple Computers, tenen un
sistema operatiu propi i una quota de mercat molt més reduïda.
\end{ja}
...
Com ja s'ha dit en la pàgina~\pageref{pg:menciofitxer}, és comú que les
dades ---de qualsevol classe: textos, programes, gràfics, so, vídeo,
etc.--- emmagatzemades en memòria secundària estiguen organitzades en
{\em fitxers}\footnote{Tamb\'{e} anomenats \emph{documents} o
  \emph{arxius}.}. Els fitxers són conjunts de dades amb un nom que
els identifica i que es manipulen ---s'obrin, es tanquen, es copien,
s'esborren--- com un tot. En discos grans, seria molt incòmode tenir
tots els fitxers un darrere l'altre; per això, és comú que els fitxers
estiguen organitzats en {\em directoris}\footnote{Tamb\'{e} anomenats
  \emph{carpetes}.}. Els directoris són fitxers especials que agrupen
els noms i les característiques d'altres fitxers; de fet, els
directoris poden contenir zero o més fitxers o tamb\'{e} zero o més
...
l'extensió sol ser curta (entre una i quatre lletres) i sol
identificar el programa que s'ha d'usar per a processar-lo o el format
en què es troben les dades que conté (per exemple, l'extensió
\texttt{.txt} identifica normalment un text senzill en format ASCII o
ANSI, vegeu l'apartat~\ref{ss:formats}; l'extensió \texttt{.exe} s'usa
per als programes d'ordinador, etc.).
...
del fitxer. Si el disc es diu \texttt{a:} (típic d'un disquet en un PC
amb sistema operatiu Windows), les trajectòries d'aquests dos fitxers
serien \texttt{a:}\carp{}\carp{prac1}\carp{iat}\texttt{iat1.txt} i
\texttt{a:}\carp{}\carp{prac1}\carp{reserva}\carp{iat}\texttt{iat1.txt},
...
que pot presentar-se en 778 formes diferents\footnote{Com, per
  exemple, els signes d'alguna escriptura ideogràfica oriental},
...
suficients, perquè $2^{10}=1.024$ (en quedarien $1024-778=246$
combinacions sense usar).

Els bits s'agrupen normalment en grups de vuit, anomenats {\em
  octets} o \emph{bytes}. Un octet pot estar, per tant, en
$2^8=256$ estats diferents. Per exemple, els caràcters i
símbols m\'{e}s comunament usats en textos se solen guardar cada
un en un octet, usant el codi ASCII\label{pg:ASCII} (\emph{American Standard Code for
  Information Interchange}), on el codi de la ``{\tt A}'' \'{e}s
``{\tt 01000001}'' o el de la ``{\tt z}'' \'{e}s ``{\tt 01111010}''
(vegeu l'epígraf~\ref{ss:formats}).  El codi ASCII és el codi bàsic
per a emmagatzemar textos; quan els textos són més rics i contenen
informació sobre tipus i grandàries de lletra, diagramació, notes a peu de
pàgina, etc., s'usen formats més avançats que s'expliquen en
l'epígraf~\ref{ss:formats}.  Un octet pot contenir, per tant, molt
poca informació (un caràcter, una instrucció senzilla del
processador central, un nombre de 0 (``{\tt 00000000}'') a 255 (``{\tt
  11111111}''), etc.).  Per exemple, un document de text com aquest
t\'{e} desenes de milers de caràcters, i una enciclopèdia,
centenars de milions. En les imatges en blanc i negre, cada punt
\'{e}s un bit; una pantalla d'ordinador en cont\'{e} m\'{e}s o menys
un milió. Si són de colors, cal m\'{e}s d'un bit per a cada
punt. Les instruccions dels programes, com el de l'exemple, que sumava
tots els enters del 1 al 10, tamb\'{e} s'emmagatzemen en octets: per
exemple, la instrucció {\tt inc A}, que incrementa l'acumulador en
1, podria ser l'octet {\tt 11010110}.
...
\end{figure}
\ifthenelse{\equal{\aquestllibre}{IAT}}{}{
\paragraph{Transductors de lletres:} La construcció d'un analitzador
morfològic d'aquesta classe és molt senzilla a partir d'expressions
que expressen les relacions d'alineament entre les formes superficials
i les lèxiques; a més, es pot relaxar la restricció que el
transductor siga determinista, per tal que durant l'anàlisi d'una
forma superficial es puga considerar més d'una forma lèxica possible.  
En concret, els analitzadors
morfològics es poden basar en \emph{transductors de lletres}
\cite{roche97b}, una subclasse dels
  transductors d'estats finits; de fet, qualsevol transductor d'estats
  finits es pot convertir sempre en un transductor de lletres. Un
  transductor de lletres es pot definir formalment com
  $T=(Q,L,\delta,q_I,F)$, on $Q$ és un conjunt finit d'\emph{estats},
  $L$ un conjunt d'\emph{etiquetes de transició}, $q_I\in Q$ és l'{\em
    estat inicial}, $F\subseteq Q$ el conjunt d'\emph{estats finals}
  i $\delta: Q \times L \rightarrow 2^Q$ és la funció de transició (on
  $2^Q$ representa el conjunt de tots els subconjunts de $Q$).
  
  El conjunt d'etiquetes de transició és $L=(\Sigma \cup \{\epsilon\})
  \times (\Gamma \cup \{\epsilon\})$ on $\Sigma$ és l'alfabet dels
  símbols d'entrada, $\Gamma$ l'alfabet dels símbols d'eixida i
  $\epsilon$ representa el símbol buit. D'acord amb aquesta definició,
  les etiquetes de transició poden ser de quatre classes:
  $(\sigma:\gamma)$, que indica que es llig el símbol
  $\sigma\in\Sigma$ i s'escriu el símbol $\gamma\in\Gamma$;
  $(\sigma:\epsilon)$, que indica que es llig un símbol però no
  s'escriu res; $(\epsilon:\gamma)$, que indica que no es llig res
  però s'escriu un símbol, i $(\epsilon:\epsilon)$, que indica que la
  transició d'estat succeeix sense llegir ni escriure. Les transicions
  de l'últim tipus no són necessàries ni convenients en el transductor
  final, però poden ser molt útils durant la construcció. S'acostuma a
  representar la cadena buida $\epsilon$ amb un zero (``{\tt 0}'').
  Es diu que un transductor de lletres és \emph{determinista} quan
  $\delta:Q\times L \rightarrow Q$, és a dir, quan totes les
  transicions que ixen d'un estat tenen etiquetes de transició
  diferents. Noteu que un transductor de lletres que siga determinista
  respecte a l'alfabet de les transicions
  $L=(\Sigma\cup\{\epsilon\})\times(\Gamma\cup\{\epsilon\})$ pot
  perfectament ser indeterminista respecte de l'alfabet d'entrada
  $\Sigma$.\footnote{És a dir, el transductor es pot trobar en més
    d'un estat per al mateix prefix del mot d'entrada i pot estar
    considerant diverses transduccions alternatives, algunes de les
    quals pot abandonar quan haja llegit un prefix més llarg del mot
    d'entrada.}
 

Es diu que una cadena de $w'\in\Gamma^*$ és una transducció d'una
cadena d'entrada $w\in\Sigma^*$ quan hi ha almenys un camí des de
l'estat inicial $q_I$ fins a un estat final en $F$ les etiquetes de
transició del qual formen la parella $w:w'$ quan es concatenen.  En
general, pot
haver-hi més d'un camí; açò s'ha d'evitar i, de fet,
s'elimina parcialment amb la determinització (vegeu més avall).
D'altra banda, pot haver-hi més d'una transducció vàlida per a una
cadena d'entrada $w$. En l'anàlisi morfològica, aquesta situació
correspondria a l'\emph{ambigüitat lèxica}). En l'anàlisi morfològica,
els símbols de $\Sigma$ són els que es troben en els textos i els de
$\Gamma$ són els necessaris per a formar els lemes i els símbols
especials que representen la informació morfològica com ara {\tt
  <nom>}, {\tt <fem>}, {\tt <2p>}, etc.


\paragraph{Minimització dels transductors de lletres:} La definició general dels transductors de lletres és completament
paral·lela a la dels autòmats finits no deterministes (AFND) i la dels
transductors de lletres deterministes, a la dels automats finits
deterministes (AFD) \com{Falten cites: Hop79a,salomaa73b}. Per això,
els transductors de lletres es poden determinitzar i minimitzar
(respecte de l'alfabet $L$) usant els algorismes existents per a AFND
i AFD \com{Falten cites: Hop79a,salomaa73b,vandesnepscheut93b}.  Les
transicions etiquetades $(\epsilon:\epsilon)$ és poden eliminar durant
la determinització usant una tècnica paral·lela a l'anomenada {\em
  clausura $\epsilon$}.

Per a minimitzar el transductor resultant, es pot usar un algorisme
com el descrit per \citet{vandesnepscheut93b}, que consta de dos
passos idèntics que es poden resumir com segueix: en cada pas, les
fletxes de les transicions del transductor de lletres s'inverteixen,
de manera que l'estat inicial és el final i al revés, i el transductor
resultant es determinitza respecte de $L$ (és a dir, es formen nous
estats amb conjunts d'estats vells de manera que la nova $\delta$ és
$\delta:Q\times L\rightarrow Q$). El transductor resultant d'aquest
procés doble d'inversió--determinització és mínim (té el mínim nombre
d'estats possible). Aquest algorisme és particularment eficient en el
cas dels transductors de lletres acíclics. A més, els dos passos tenen
interpretacions senzilles: el primer pas uneix terminacions comunes
(troba regularitats en la transducció dels sufixos) i el segon uneix
començaments comuns (troba regularitats en la transducció dels
prefixos).

Els transductors en qüestió són capaços d'entregar totes les anàlisis
morfològiques possibles d'un homògraf (vegeu la secció
\ref{s3:alcat}).
